...
class Stream_plethysm(Stream_binary):
    ...
    Check that degree one elements are treated in the correct way::
        ...
        sage: r_s - f_s(g_s, include=[])                                                # needs lrcalc_python sage.modules
        (a2*b1^2-a2*b1)*p[2] + (a2*b111^2-a2*b111)*p[2, 2, 2] + (a2*b21^2-a2*b21)*p[4, 2]

        sage: r2 = Stream_plethysm(f, g, True, p, include=[])                           # needs lrcalc_python sage.modules
        sage: r_s - sum(r2[n] for n in range(2*(r_s.degree()+1)))                       # needs lrcalc_python sage.modules
        (a2*b1^2-a2*b1)*p[2] + (a2*b111^2-a2*b111)*p[2, 2, 2] + (a2*b21^2-a2*b21)*p[4, 2]
    """
