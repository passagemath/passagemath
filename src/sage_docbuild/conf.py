...
def linkcode_resolve(domain, info):
    ...
    if info['module']:
        ...
        if 'fullname' in info:
            ...
            anchor = ''
        return f"{source_repository}blob/main/src/{filename}{anchor}"
    return None
...
is_for_develop = github_ref.startswith('refs/heads/main')
...
def add_page_context(app, pagename, templatename, context, doctree):
    ...
    if 'reference' in path1 and not path1.endswith('reference'):
        ...
        if pagename.startswith('sage/'):
            ...
            context['theme_source_view_link'] = os.path.join(source_repository, 'blob/main/src', '{filename}')
            context['theme_source_edit_link'] = os.path.join(source_repository, 'edit/main/src', '{filename}')
