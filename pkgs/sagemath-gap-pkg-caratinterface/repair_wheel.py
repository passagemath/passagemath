...
with InWheel(wheel, wheel):
    ...
    for dir in GAP_ROOT_PATHS.split(';'):
        ...
        if (Path(dir) / pkgdir).exists():
            found = True
            command = f'set -o pipefail; (cd {shlex.quote(str(parent))} && tar cf - --exclude "*/caratinterface/gen" --exclude "*/caratinterface/libcaratinterface" --exclude "*/caratinterface/doc" --exclude "*/caratinterface/src" {name}/{pkgdir}) | tar xvf -'
